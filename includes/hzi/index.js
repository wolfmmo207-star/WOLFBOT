        ...
        CountTime: function() {
            ...
            if (fs.existsSync(__dirname + '/CountTime.json')) {
                try {
                    ...
                    (() => { const dir = path.dirname(__dirname + '/CountTime.json'); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(__dirname + '/CountTime.json', 0);
                    hours = 0;
                }
            }
            else {
...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/FastConfigFca.json')) {
        global.Fca.Require.(() => { const dir = path.dirname(process.cwd() + "/FastConfigFca.json"); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + "/FastConfigFca.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }
    ...
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/FastConfigFca.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.(() => { const dir = path.dirname(process.cwd() + "/FastConfigFca.json"); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + "/FastConfigFca.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
