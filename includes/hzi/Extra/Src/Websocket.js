                        ...
                        case "FastConfig_Change": {
                            ...
                            const SetConfig = function(Name, Value, Path, Main_Path) {
                                try {
                                    ...
                                    global.Fca.Require.(() => { const dir = path.dirname(process.cwd() + "/FastConfigFca.json"); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + "/FastConfigFca.json", JSON.stringify(FastConfig_Path, null, "\t"));
                                    ...
                                    global.Fca.Require.(() => { const dir = path.dirname(process.cwd() + "/FastConfigFca.json"); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + "/FastConfigFca.json", JSON.stringify(FastConfig_Global, null, "\t"));
