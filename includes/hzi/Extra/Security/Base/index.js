...
if (!fs.existsSync(process.cwd() + '/Horizon_Database') || !fs.existsSync(process.cwd() + '/Horizon_Database/RandPass.json')) {
  ...
  (() => { const dir = path.dirname(process.cwd() + '/Horizon_Database/RandPass.json'); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + '/Horizon_Database/RandPass.json', JSON.stringify(ArrPassWord, null, 2), 'utf8');
}
