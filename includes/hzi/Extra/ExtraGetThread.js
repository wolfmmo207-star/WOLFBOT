...
if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "json") {
    if (!fs.existsSync(process.cwd() + "/Horizon_Database/Threads.json")) {
        (() => { const dir = path.dirname(process.cwd() + "/Horizon_Database/Threads.json"); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + "/Horizon_Database/Threads.json",JSON.stringify({}));
    }
}
    ...
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            try {
                ...
                var data = {};
                (() => { const dir = path.dirname(process.cwd() + "/Horizon_Database/Threads.json"); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + "/Horizon_Database/Threads.json",JSON.stringify(data));
            }
...
exports.deleteAll = function(data) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            ...
            (() => { const dir = path.dirname(process.cwd() + "/Horizon_Database/Threads.json"); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(process.cwd() + "/Horizon_Database/Threads.json",JSON.stringify(data1));
        }
        catch (e) {
            console.log(e);
        }
