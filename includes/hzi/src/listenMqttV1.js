...
function LogUptime() {
    ...
    if (global.Fca.Require.fs.existsSync(join(__dirname, '../CountTime.json'))) {
        ...
        global.Fca.Require.(() => { const dir = path.dirname(join(__dirname); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(join(__dirname, '../CountTime.json'), String(Number(uptime) + Time1), 'utf8');
    }
    else {
        var Time1 = 0;
        global.Fca.Require.(() => { const dir = path.dirname(join(__dirname); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); return fs.writeFileSync(join(__dirname, '../CountTime.json'), String(Number(uptime) + Time1), 'utf8');
