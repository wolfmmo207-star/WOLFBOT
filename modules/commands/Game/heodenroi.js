...
module.exports.onLoad = async () => {
    const dir = __dirname + `/FolderGame/heodenroi/datauser/`;
    const _dir = __dirname + `/FolderGame/heodenroi/datauser/`;
    const __dir = __dirname + `/FolderGame/heodenroi/cache/`;
...
module.exports.checkPath = function (type, senderID) {
    const pathGame = path.join(__dirname,'FolderGame', 'heodenroi', 'datauser', `${senderID}.json`);
    const pathGame_1 = require("./FolderGame/heodenroi/datauser/" + senderID + '.json');
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        writeFileSync( __dirname + `/FolderGame/heodenroi/cache/heodenroi.png`, Buffer.from(download, "utf-8"));
        images.push(createReadStream(__dirname + `/FolderGame/heodenroi/cache/heodenroi.png`));
    return images
}

module.exports.run = async function ({ api, event, args, Users, Currencies }) {
    const { threadID, messageID, senderID } = event;
    const pathData = path.join(__dirname, 'FolderGame','heodenroi', 'datauser', `${senderID}.json`);
    switch (args[0]) {
        case 'register':
        case '-r': {
        ...
        case 'spin': {
            ...
            const data = readdirSync(__dirname + `/FolderGame/heodenroi/datauser`);
            if(i == 3) {
                if(data.length < 4) return api.sendMessage(`[ HDR ] - Cần ít nhất có 3 người chơi trên server để trộm`, threadID, messageID);
                const dem = [];
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        dem.push(require(`./FolderGame/heodenroi/datauser/${i}`));
                    }
                }
                ...
                for (let i of data) { 
                    if(i != `${senderID}.json`) {
                        var o = require(`./FolderGame/heodenroi/datauser/${i}`);
                        p.push(o)
        ...
        case 'xaydung': {
            ...
            var a = require(`./FolderGame/heodenroi/datauser/${senderID}.json`);
...
module.exports.handleReply = async ({ event, api, Currencies, handleReply, Users }) => {
    ...
    switch (handleReply.type) {
        case 'steal': {
            ...
            var dataUser = require(`./FolderGame/heodenroi/datauser/${senderID}`);
